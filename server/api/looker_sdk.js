...
export const getLookerSession = async () => {
  if (!lookerSession || !lookerSession.activeToken.isActive()) {
    ...
    console.log("Config: ", api_url, client_id, client_secret)
    try {
      ...
      console.log("Looker Session settings: ", lookerSettings)
      lookerSession = new NodeSession(lookerSettings)
      console.log("Looker session: ", lookerSession)
      const userrr = await lookerSession.login()
...
export const getEmbedUserApiSession = async (external_user_id) => {
  const sdk = new Looker40SDK(await getLookerSession())
  const embedUserCredentials = await sdk.ok(sdk.user_for_credential("embed", external_user_id));
  const embedUserToken = await sdk.login_user(embedUserCredentials.id.toString());
  const embedUserApiSession = {
    user_token: embedUserToken.value,
    token_last_refreshed: Date.now(),
  };
  return { ...embedUserApiSession } ;
}
